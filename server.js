...
setInterval(() => {
  http.get(`http://tastydf7.glitch.me/`);
}, 280000);
...
client.on("message", msg => {
  //WESO#0001
  if (msg.content === "ابراهيم") {
    //WESO#0001
    msg.reply("**ابراهيم  مات الله يرحمة**"); //WESO#0001
  }
});
...
client.on("message", msg => {
  ...
  if (msg.content === "انس") {
    //WESO#0001
    msg.reply("**انس تم تهكيرة**"); //WESO#0001
  }
});
  ...
  if (message.content.startsWith(prefix + "v2min")) {
    ...
    if (!nam)
      return message.channel
        ...
        .then(msg => msg.delete(10000)); //WESO#0001
    message.guild
      .createChannel(nam, "voice")
      .then(c => setTimeout(() => c.delete(), 120000)); //WESO#0001
    message.channel
...
client.on("message", message => {
  ...
  if (message.content.startsWith(prefix + "emoji")) {
    ...
    if (emojiid.length < "18" || emojiid.length > "18" || isNaN(emojiid))
      return message.channel.send(`- Usage  //WESO#0001
${prefix}emoji <EmojiID>`);
    else
      message.channel.send(
        "This is the emoji that you requested:-", //WESO#0001
        {
...
client.on("message", async message => {
  ...
  - \`${prefix}clan room\`: لعمل روم شات او كتابي بأسم الكلان ( صاحب الكلان فقط ) ء`
    ...
    if (args[1] && args[1] === "invite") {
      //WESO#0001
      if (!system[author.id])
        return message.channel.send("**# أنت لست بكلان**"); //WESO#0001
      ...
      if (clans[clan].members.includes(mention.id))
        return message.channel.send("**# هذا العضو بالكلان بالفعل**"); //WESO#0001
    ...
    if (args[1] && args[1] === "join") {
      ...
      let clanName = message.content
        ...
        .join(" "); //WESO#0001
      if (system[author.id].clan !== "None")
        return message.channel.send("**# يجب أن لا تكون بكلان**"); //WESO#0001
      if (!args[2])
        return message.channel.send("**# يجب عليك كتابة اسم الكلان**"); //WESO#0001
      if (!clans[clanName])
        return message.channel.send("**# هذا الكلان غير موجود**"); //WESO#0001
      if (clans[clanName].applylist.includes(message.author.id))
        return message.channel.send("**# لقد قدمت على دخول هذا الكلان مسبقا"); //WESO#0001
    ...
    if (args[1] && args[1] === "promote") {
      ...
      if (system[mention.id].clan === "None")
        return message.channel.send("**# هذا الشخص ليس بكلان**"); //WESO#0001
      if (!clans[system[author.id].clan].members.includes(mention.id))
        return message.channel.send("**# هذا الشخص ليس بالكلان**"); //WESO#0001
      if (clans[system[author.id].clan].admins.includes(mention.id))
        return message.channel.send("**# هذا العضو لديه ادمن بالفعل**"); //WESO#0001
      if (mention.id === message.author.id)
        return message.channel.send("**# لا يمكنك اعطاء نفسك ترقية**"); //WESO#0001
    ...
    if (args[1] && args[1] === "demote") {
      ...
      if (clans[system[author.id].clan].creator !== message.author.id)
        return message.channel.send("**# هذا الأمر لضاحب الكلان فقط**"); //WESO#0001
      if (!mention)
        return message.channel.send(
          "**# يجب عليك منشنة عضو بالكلان لأعطائه الترقية**"
        ); //WESO#0001
      ...
      if (!clans[system[author.id].clan].members.includes(mention.id))
        return message.channel.send("**# هذا الشخص ليس بالكلان**"); //WESO#0001
      if (!clans[system[author.id].clan].admins.includes(mention.id))
        return message.channel.send("**# هذا الشخص ليس ادمن بالكلان**"); //WESO#0001
      if (mention.id === message.author.id)
        return message.channel.send("**# لا يمكنك اعطاء نفسك ترقية**"); //WESO#0001
      //WESO#0001
...
client.on("message", message => {
  var args = message.content.substring(prefix.length).split(" ");
  if (message.content.startsWith("مسح الكل")) {
    if (!message.channel.guild)
      return message.reply("**❌ اسف لكن هذا الامر للسيرفرات فقط **");
    if (!message.member.hasPermission("MANAGE_MESSAGES"))
      return message.reply("**⚠  لا يوجد لديك صلاحية لمسح الشات**");
...
const devs = ["601533400528191509", "620399766203006980"];
...
『-credits===>لمعرفة كم معك من المال
...
client.on("ready", async () => {
  client.channels
    .find(ch => ch.id === "649759238821773334" && ch.type === "voice")
    .join();
});
  ...
  if (command === `play`) {
    ...
    if (!permissions.has("EMBED_LINKS")) {
      return msg.channel.sendMessage(
        "Ido not have permission  ``EMBED_LINKS`` "
      );
    }
    voiceChannel.join();

    if (!args[1])
      return msg.channel.send(`:bulb: Play Commands:
...
\`\`${prefix}play <URL>\`\` - plays the provided song, playlist, or stream`);
    ...
    if (url.match(/^https?:\/\/(www.youtube.com|youtube.com)\/playlist(.*)$/)) {
      ...
      return msg.channel.send(
        `**${playlist.title}**, Just added to the queue!`
      );
    } else {
      try {
    ...
    serverQueue.connection.dispatcher.end(
      `:notes: Skipped asdasd - <@${msg.author.id}>`
    );
    ...
    if (!serverQueue)
      return msg.channel.send(" The bot is not playing :interrobang:");
    if (!args[1])
      return msg.channel.send(
        `:speaker: Current volume is **${serverQueue.volume}**`
      );
    if (args[1] > 100 || args[1] < 10)
      return msg.channel.send(
        ":no_entry_sign: Volume must be a valid integer between ``10`` and ``100``"
      );
    ...
    return msg.channel.send(
      `:loud_sound: Volume changed on \`\`${args[1]}\`\``
    );
  } else if (command === `np`) {
    if (!serverQueue) return msg.channel.send("There is no Queue!");

    return msg.channel.send(
      `:arrow_forward: **${serverQueue.songs[0].title}**`
    );
    ...
    const embedqu = new Discord.RichEmbed()
      .setTitle(
        `:notes: Current Queue | ${serverQueue.songs.length - 1} entries`
      )
      .setDescription(
        `
        ${serverQueue.songs
          .map((song, index) =>
            index == 0 ? null : `**[${index}]** ${song.title} - `
          )
          .join("\n")}`
      )
      .setColor("#4f545c");
    return msg.channel
      .send(`:arrow_forward: **${serverQueue.songs[0].title}**`)
      .then(a => {
...
async function handleVideo(video, msg, voiceChannel, playlist = false) {
  ...
  if (!serverQueue) {
    ...
    if (playlist) return undefined;
      ...
      return msg.channel.send(
        ` :notes: **${song.title}** Added to **Queue** !`
      );
  }
  return undefined;
}
...
client.on("message", message => {
  ...
  var emb = new Discord.RichEmbed()
    ...
    .setDescription(
      `**Message from ${message.author} in the bot dm**\n\`\`\`apache\nMessage; ${message.content}\`\`\``
    )
...
client.on("message", (message) => {
    if(message.content.startsWith(prefix+"yahoo")) {
        message.channel.send(JSON.stringify({
            email: Math.random().toString(36).slice(4).trim()+"@yahoo.com",
            password: Math.random().toString(36).slice(4).trim()
        }))
    }
});
...
client.on("message", async message => {
  if(message.author.id !== "601533400528191509") return;
...
Bot Prefix: \`${prefix}\`\n\nBot Owner: <@601533400528191509>**`) // ايديك هنا
...
client.on('message', message => {
    ...
    ytScraper.channelInfo("https://www.youtube.com/channel/UCMbjRjsgedv41D3hv_wzVrw?view_as=subscriber").then(data => {
        const embed = new Discord.RichEmbed()
        .setColor("#36393e")
...
if (message.content === '-invite') {
      message.author.send('  البوت خاص بسيرفر دبي ولا يمكنك استخدامة').catch(e => console.log(e.stack));
...
client.on('message', message => {
  ...
  const embed = new Discord.RichEmbed()
    ...
    .setDescription(`suport server **[here](https://discord.gg/fdsNC5H)**`)
    .setColor('RANDOM');
...
client.on('message', function(message) {
	let mohammad = client.channels.get("649761133221380116");
	let gamer = message.content.split(" ");  
	if(message.author.bot) return;
  if(message.content.startsWith("ابراهيم")) return mohammad.send("`هناك ناس ذكرت اسمك هل تريد رأيت الرسالة ؟`").then(msg => {
    msg.react("✅")
    msg.react("❌")
.then(() => msg.react('❌'))
.then(() =>msg.react('✅'))
...
client.on("guildMemberAdd", async member => {
	     let moment = require("moment"),  
		  ...
		  if(date < 5) {
			  member.ban(" الحساب اقل من 5 أيام ")
		  }
});
...
client.on("message", msg=>{
let id = "620399766203006980"; // ايديك
let role = "Dubai seller"; // اسم رتبة الفيب
let Price = 10; // السعر
let Price2 = 9
...
//CopyRights ToxicCodes 04/28/2019 🌠☭ 🕅ØŇŞŦ€Ř ҜƗŇᎶ 👺❦❧#8722
client.login("NjE3MTAxMjY1NzI1NTU0Njkw.Xez1Xw.NtFq_dMav4YRFFsHJpwO-BS2B5E");
